...
whisk=plak(k,1:2);
if plots==1
    fill(whisk(:,1),whisk(:,2),colorfence,'LineStyle','-','FaceColor','none','EdgeColor',colorfence,'LineWidth',.6)
    ...
    bagq=bag(q,1:2);
    fill(bagq(:,1),bagq(:,2),colorbag,'LineStyle','-','FaceColor',colorbag,'EdgeColor',colorbag,'LineWidth',.6,'FaceAlpha',.3)%This line should be placed after the command plot(data...) to only %plot %the data outside the bag
    axis square
    if databag==1
        plot(data1(:,1),data1(:,2),'o','MarkerFaceColor','k','MarkerEdgeColor','none','Markersize',3)
    end
    if datafence==1
        plot(data2(:,1),data2(:,2),'o','MarkerFaceColor','k','MarkerEdgeColor','none','Markersize',3)
    end
    plot(outl(:,1),outl(:,2),'o','MarkerFaceColor','k','MarkerEdgeColor','none','Markersize',3)
   
    %plot(tukm(1),tukm(2),'o','MarkerFaceColor','y','MarkerEdgeColor','none','MarkerSize',3); % plots the median
    
    mean_x = mean(S(:,1));
    mean_y = mean(S(:,2));
    plot(mean_x,mean_y, 'o','MarkerFaceColor','y','MarkerEdgeColor','none','MarkerSize',3); % plots the mean

  %  plot(tukm(1),tukm(2),'+k','Markersize',6)
%     switch type
%         case 'ao'
...
% Fix: only check collinearity when we have enough points and use scalar extraction
if numel(wx2) >= 2 && numel(wy2) >= 2 && ~(all(wx2==wx2(1)) || all(wy2==wy2(1)))
    R2 = corrcoef(wx2, wy2);
    if R2(1,2) > 0.98
        % Instead of erroring, add small jitter to break collinearity
        wx2 = wx2 + randn(size(wx2)) * 1e-8;
        wy2 = wy2 + randn(size(wy2)) * 1e-8;
        Dk2 = [wx2, wy2];
        kount2 = size(Dk2, 1);
    end
end
