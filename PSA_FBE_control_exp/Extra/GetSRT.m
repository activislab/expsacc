...
data_path = '/home/kaneda/Documents/Projects/PSA_FBA_control_exp';
...
% sac(1:num,8)   saccade magnitude        ()
        ...
        for l = 1:size(s.eyeraw,1)
            ...
            timevec = s.time;  % already in ms
            baseline_idx = timevec >= 1 & timevec <= 170;

            %             if sum(abs(veltemp(:,1))) > 0
            %                 sactemp = microsacc(squeeze(s1.eyeraw(l,1:2,:))',veltemp,VTHRES,MINDUR);
            %             end

            if sum(abs(veltemp(:,1))) > 0

                % --- compute threshold ONLY from baseline ---
                vel_baseline = veltemp(baseline_idx,:);

                msdx = sqrt( median(vel_baseline(:,1).^2) - (median(vel_baseline(:,1)))^2 );
                msdy = sqrt( median(vel_baseline(:,2).^2) - (median(vel_baseline(:,2)))^2 );

                radiusx = VTHRES * msdx;
                radiusy = VTHRES * msdy;

                % --- pass fixed radius to modified function ---
                sactemp = microsacc_fixed_radius( ...
                    squeeze(s.eyeraw(l,1:2,:))', veltemp, radiusx, radiusy, MINDUR);
            end
            ...
            valid = find(abs(sactemp(:,8)) > min_amp);

            if ~isempty(valid)
                macrosacvec(l,:) = sactemp(valid(1),:); % take FIRST valid saccade
            else
